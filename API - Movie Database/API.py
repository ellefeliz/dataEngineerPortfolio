...
class Graph:
    ...
    def add_node(self, id: str, name: str) -> None:
        ...
        """
        res = [item for item in self.nodes if item[0] == id]
        if len(res) == 0:
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        res = [item for item in self.edges if item[0] == source and item[1] == target]
        res2 = [item for item in self.edges if item[1] == source and item[0] == target]

        #if len(res) == 0 and len(res2) == 0 and (target != source):
        if (edge not in self.edges) and (rev not in self.edges) and (target != source):
    ...
    def max_degree_nodes(self) -> dict:
        ...
        for node in self.edges:
            if node[0] in dct.keys():
                dct[node[0]] += 1
            else:
                dct[node[0]] = 1

            if node[1] in dct.keys():
                dct[node[1]] += 1
            else:
                dct[node[1]] = 1
    ...
    def least_degree_nodes(self) -> (int, int):
        """
        Return the node(s) with the least degree
        Return multiple nodes in the event of a tie
        Format is a dict where the key is the node_id and the value is an integer for the node degree
        e.g. {'a': 8}
        or {'a': 22, 'b': 22}
        ...
        return (total, len(rtn))


    def count_leaf(self) -> int:
        dct = {}
        for node in self.edges:
            if self.nodes[0] in dct.keys():
                dct[node[0]] += 1
            else:
                dct[node[0]] = 0

            if self.nodes[1] in dct.keys():
                dct[node[1]] += 1
            else:
                dct[node[1]] = 0
        cnt = 0
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        load = {}
        url = "/3/movie/"+str(movie_id)+"/credits?api_key="+"MUST ADD YOUR API KEY HERE"+&language=en-US"
        #print(url)
        connection.request("GET",url,load)
        response = connection.getresponse()
        data = response.read().decode('utf-8')
        rtn = []
        if response.status == 200:
            json_data = json.loads(data)['cast']
            #print(len(json_data))
            if len(json_data)>0:
                rslt_1 = []
                rslt_2 = []
                ...
                if exclude_ids is None:
                    for member in json_data:
                        dct = {}
                        dct['id'] = member['id']
                        dct['name'] = member['name']
                        dct['credit_id'] = member['credit_id']
                        dct['order'] = member['order']
                        filtered_cast.append(dct)
                else:
                    for member in json_data:
                        dct = {}
                        if member['id'] not in exclude_ids:
                            dct['id'] = member['id']
                            dct['name'] = member['name']
                            dct['credit_id'] = member['credit_id']
                            dct['order'] = member['order']
                ...
                if limit is None:
                    ...
                    for member in filtered_cast:
                        dct = {}
                        #print(int(member['order']))
                        if int(member['order']) < limit:
                            #print("got it")
                            dct['id'] = member['id']
                            dct['name'] = member['name']
                            dct['credit_id'] = member['credit_id']
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        connection = http.client.HTTPSConnection(f"api.themoviedb.org")
        load = {}

        url = f"/3/person/{person_id}/movie_credits?api_key="+"MUST ADD YOUR API KEY HERE"+"&language=en-US"
        #print(url)
        ...
        if response.status == 200:
            ...
            if len(json_data) > 0:
                ...
                if vote_avg_threshold is not None:
                    vote_avg = vote_avg_threshold

                for cast in json_data:
                    dct = {}
                    if float(cast['vote_average']) >= vote_avg:
                        dct['id'] = cast['id']
                        dct['title'] = cast['title']
                        dct['vote_avg'] = cast['vote_average']
                        rtn.append(dct)
...
def return_argo_lite_snapshot()->str:
    ...
    """
    return 'https://poloclub.github.io/argo-graph-lite/#13130317-fba3-4bf1-9325-a418e8732136'
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='MUST ADD YOUR API KEY HERE')
    api_key = tmdb_api_utils.api_key
    ...
    nodes_lst = []
    credits = tmdb_api_utils.get_movie_credits_for_person(person_id='2975', vote_avg_threshold=8.0)
    for cr in credits:
        cast = tmdb_api_utils.get_movie_cast(str(cr['id']), limit=3)
        for member in cast:
            if member['id'] != '2975':
                graph.add_node(str(member['id']), member['name'].replace(',', ' '))
                graph.add_edge('2975',str(member['id']))
                nodes_lst.append(str(member['id']))
    ...
    #nodes_lst.append('2975')

    for i in range(2):
        cnt = 0
        tmp_nodes = []
        tmp_nodes = nodes_lst.copy()
        print("loop :", i, "\n", "new nodes :", len(tmp_nodes), "\n", "total nodes :", len(graph.nodes), "\n", "total edges :", len(graph.edges))
        nodes_lst = []
        #print(tmp_nodes)

        for node in tmp_nodes:
            credits = tmdb_api_utils.get_movie_credits_for_person(person_id=str(node), vote_avg_threshold=8.0)
            #print(credits)
            for cr in credits:
                cast = tmdb_api_utils.get_movie_cast(str(cr['id']), limit=3)
                #print(cast)
                for member in cast:
                    #if member['id'] != node:
                    graph.add_node(str(member['id']), member['name'].replace(',', ' '))
                    graph.add_edge(str(node),str(member['id']))
                    if str(member['id']) not in nodes_lst:
                        nodes_lst.append(str(member['id']))
    ...
    #print("Numer of leaf",graph.count_leaf())
    print(graph.least_degree_nodes())
    graph.write_edges_file()
    graph.write_nodes_file()
